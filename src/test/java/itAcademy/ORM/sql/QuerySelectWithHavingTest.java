...
import itAcademy.ORM.sql.subclauses.HavingClause;
...
public class QuerySelectWithHavingTest {
    ...
    @Test
    public void shouldSelectCOUNT_FromTableWithGroupByWithHaving() {
        Query query = new Query(QueryType.SELECT);
        query.addTable("std");
        query.addSubclause(SubclauseType.GROUP_BY, new GroupByClause("last_name"));
        query.addSubclause(SubclauseType.HAVING, new HavingClause(new DataField("COUNT(gpa) > 1")));
        new QueryBuilder(query).fieldOp(QueryFieldOperation.COUNT, "gpa");
        ...
        try {
            ...
            String actual = result.getString(1) + " " + result.getString(2)
                    + " " + result.getString(3);
            String expected = "2 Dolenko Yulia";
